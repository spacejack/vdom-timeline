...
/* Extending Promise - possible only when browser has ES6 Promse & class support
export class Timeline<T> extends Promise<T> {
	canceled: Promise<void>
	protected _pauseEmitter: Emitter
	protected _resumeEmitter: Emitter
	protected _cancel: (t?: void | undefined) => void

	constructor(f: TimelineCallback<T>) {
		const [canceled, cancel] = createToken<void>()
		const pauseEmitter = Emitter()
		const resumeEmitter = Emitter()
		const delay = (obj: number | Howl | TimelinePromise<any>) => Delay(
			canceled, pauseEmitter, resumeEmitter, obj
		)
		super(resolve => {
			f(delay, cancel, canceled).then(resolve)
		})
		this.then(r => {})
		this._pauseEmitter = pauseEmitter
		this._resumeEmitter = resumeEmitter
		this._cancel = cancel
		this.canceled = canceled
	}

	pause() {
		this._pauseEmitter.emit()
	}

	resume() {
		this._resumeEmitter.emit()
	}
